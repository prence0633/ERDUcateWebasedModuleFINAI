...
const rationalizations = {
  ...
  q7: "Correct: Handwritten random notes are unstructured, not structured.",
  q8: "Correct: Spreadsheets with columns are an example of structured data.",
  q9: "Correct: Photos without labels are unstructured unless metadata/labels are provided.",
  q10: "Correct: Structured data speeds up finding and comparing records.",
...
form.addEventListener('submit', function(e){
  ...
  for (let i=6;i<=10;i++){
    ...
    if (selected === correctAnswers[name]) {
      ...
      // if user didn't select, mark wrong but still show rationalization
      li.textContent = `❌ Q${i}: ${rationalizations[name]}`;
    }
    rationalizationList.appendChild(li);
  }
